        ...
        (
            <div className='h-full overflow-scroll relative backdrop-blur-2xl'>
                ...
                <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
                    <img src={selectedUser.profilePic || assests.avatar_icon} alt='profile image' className='w-8 rounded-full' />
                    ...
                    {Array.isArray(messages) && messages.map((msg, index) => (
                        <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
                            ...
                            <div className='text-center text-xs'>
                                <img src={msg.senderId == authUser._id ? authUser?.profilePic ||assests.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} className='w-7 rounded-full' />
                ...
                <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
                    <div className='flex flex-1 items-center bg-gray-100/12 px-3 rounded-full'>
                        <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>e.key==='Enter'? handleSendMessage(e):null} type='text' placeholder='Send a message' className='flex flex-1 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-500'/>
                        <input onChange={handleSendImage} type='file' id='image' accept='image/png ,image/jpeg' hidden/>
                        <label htmlFor='image'>
                            <img src={assests.gallery_icon} alt='' className='w-5 mr-2 cursor-pointer'/>
                        </label>
                    </div>
                    <img onClick={handleSendMessage} src={assests.send_button} alt='' className='w-7  cursor-pointer'/>
                </div>
            </div>
        ) :
        (
